...
| **Analytical (Non-PII)** | `sectorTags`, `role`, `tenantId`, `createdAt`, `npsScore`, `starRating`, `isActive`, `rematchCount`, `expectationCategories`, `timeCommitment`, `mentiNeeds`, `mentorStrengths`, `supportApproach`, `priorityValue`, `UserProfile.skillTags`, `UserProfile.goalTags`, `UserProfile.industryCode`, `UserProfile.yearsExp` | Safe for aggregate reporting. May appear in KPI dashboards. Must NOT be linked to specific user identity in any exported report. `mentiNeeds`/`mentorStrengths`/`supportApproach`/`priorityValue` (üç soru S1/S2/S3): aggregate KPI güvenli AMA §10.3 görünürlük — S1 (`mentiNeeds`) karşı tarafa SEÇİM ekranında GÖSTERİLMEZ, admin'e yalnız k-anonim TOPLU. |
